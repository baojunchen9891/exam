...
(function (global) {
  ...
  // 档位判定
  function tier(P) {
    if (P < 0.4) return { key: 'high', label: '通过风险高', color: 'var(--red)', advice: '建议先系统补弱，再考虑报考' };
    if (P < 0.65) return { key: 'mid', label: '中等', color: '#eab308', advice: '有希望，针对性补弱可明显提升' };
    if (P < 0.85) return { key: 'stable', label: '较稳', color: 'var(--green)', advice: '基础较扎实，冲刺高分' };
    return { key: 'safe', label: '稳过', color: 'var(--green)', advice: '保持手感即可' };
  }
