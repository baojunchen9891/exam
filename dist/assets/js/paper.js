...
(function () {
  ...
  function renderHead() {
    ...
    $("#headBox").innerHTML = `<h2 class="section-title" style="margin-bottom:6px">${E.esc(PAPER.title)}</h2>
      <div class="meta" style="color:var(--color-text-meta)">共 ${PAPER.count} 题 · 时长 ${m.durationMin || 60} 分钟 · 总分 ${m.totalScore || 100} · 及格 ${m.passScore || 60}</div>`;
  ...
  function bind() {
    ...
    $("#paperContent").addEventListener("click", (e) => {
      ...
      b.innerHTML = on ? '<i data-lucide="star"></i> 已收藏' : '<i data-lucide="star" class="icon-star-empty"></i> 收藏';
      $("#stat").textContent = `已收藏 ${E.favs().filter(x => x.paperId === pid).length} 题`;
  ...
  function downloadWord() {
    ...
    body += `<p style="text-align:center;color:var(--color-slate-500)">共 ${PAPER.count} 题</p>`;
