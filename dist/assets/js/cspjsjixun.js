...
(function () {
  "use strict";
  var E = EXAM; // 复用 common.js 全局账号对象（登录守卫 requireLogin 等）
  ...
  function renderLanding() {
    getJSON(BASE + "cspjsjixun_index.json").then(function (idx) {
      ...
      idx.modules.forEach(function (m) {
        ...
        a.innerHTML =
          '<div class="csp-card-ico"><i data-lucide="book-open"></i></div>' +
          "<h3>" + esc(m.title) + "</h3>" +
          '<p>' + esc(m.desc) + "</p>" +
          '<div class="csp-card-meta"><span><i data-lucide="book-open"></i> ' + m.kp + " 考点</span></div>";
        mg.appendChild(a);
      });
      // 工具
      var tg = $("#cspToolGrid");
  ...
  function renderModule() {
    ...
    getJSON(BASE + "cspjsjixun_index.json").then(function (idx) {
      ...
      $("#cspModMeta").innerHTML = '<span class="chip"><i data-lucide="book-open"></i> ' + m.kp + " 考点</span>";
  ...
  function renderKnowledge() {
    ...
    getJSON(BASE + "knowledge/" + id + ".json").then(function (d) {
      ...
      $("#cspLeafFoot").innerHTML = '<a class="csp-btn-ghost" href="/cspjsjixun/sprint.html"><i data-lucide="pencil"></i> 做几道相关题巩固一下</a>';
      ...
      if (t.type === "content") {
        ...
        return getJSON(BASE + t.bank + "_questions.json").then(function (qd) {
          ...
          body.innerHTML =
            ...
            '<a class="csp-btn" href="/cspjsjixun/sprint.html?bank=' + t.bank + '"><i data-lucide="play"></i> 开始练习（' + n + " 题）</a>" +
            '<a class="csp-btn-ghost" href="/cspjsjixun/sprint.html?bank=' + t.bank + '&mode=mock"><i data-lucide="timer"></i> 全真模考模式</a>' +
            "</div>";
        });
      }
  ...
  function buildQuizUI(body, qs, bank, mode) {
    ...
    function renderQ() {
      ...
      if (mode === "mock") {
        head.innerHTML += '<div class="csp-quiz-timer" id="cspTimer"><i data-lucide="timer"></i> 不限时</div>';
      }
      ...
      submit.onclick = function () {
        ...
        var ok = checkAns(sel);
        state.answered++;
        if (ok) { state.right++; fb.innerHTML = '<span class="csp-ok"><i data-lucide="check-circle"></i> 回答正确</span>'; }
        else {
          state.wrong++; fb.innerHTML = '<span class="csp-bad"><i data-lucide="x-circle"></i> 回答错误，正确答案：<b>' + esc(q.answer) + "</b></span>";
          wrongAdd(q);
        }
    ...
    function renderResult() {
      ...
      r.innerHTML =
        '<h2>练习完成 <i data-lucide="party-popper"></i></h2>' +
        ...
        '<button class="csp-btn" id="cspRetry"><i data-lucide="rotate-cw"></i> 再来一次</button>' +
  ...
  /* ---------- 路由 ---------- */
  function boot() {
    // 【CSP 集训】模块统一登录校验：未登录或账户未审批通过，直接跳登录页（与 exam/paper/records 一致）
